...
const validateActor = ({ avatar, name, about, gender }) => {
  ...
  if (!gender.trim()) return { error: "Select a gender!" };
  if (avatar && !avatar.type?.startsWith("image"))
    return { error: "Invallid image/avatar file" };

  return { error: null };
};

const ActorForm = ({ title, initialState, btnTitle, onSubmit, busy }) => {
  ...
  return (
    ...
    >
      <div className="flex justify-between mb-3 items-center">
        <h1 className="font-semiold text-xl dark:text-white text-primary">
          {title}
        </h1>
        <button
